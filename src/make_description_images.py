...
class ImageLayer:
    ...
    def __post_init__(self):
        ...
        over_img = resize_image(
            read_image(self.image_path), height=self.height, width=self.width
        )
        self._img = get_synthetic_image(
            under_image=under_img, over_image=over_img, yx=(0, 0)
        )
...
class Layer:
    ...
    def set_layer(self, layer_val: str):
        ...
        if self.description_type == DescriptionType.image:
            self._img = ImageLayer(
                layer_val, self.height, self.width, self.background_color
            )
        elif self.description_type == DescriptionType.string:
            self._img = StringLayer(
                layer_val, self.height, self.width, self.background_color
            )
...
class DescriptionImage:
    ...
    def generate_by_project_json(cls, json_path: Path):
        ...
        for layer_name, layer_setting_dict in description_image_dict["layers"].items():
            name: str = layer_name
            description_type: DescriptionType = getattr(
                DescriptionType, layer_setting_dict["description_type"]
            )
            ...
            layer: Layer = Layer.generate_by_2_coordinate(
                name, c0, c1, description_type, background_color
            )
            layers.append(layer)
...
class DescriptionImagesProject:
    ...
    def initialize_description_image(self) -> None:
        ...
        self._description_image_base = DescriptionImage.generate_by_project_json(
            json_path
        )
    ...
    def set_values(self) -> None:
        ...
        for _ in read_tsv(tsv_path, read_header=False):
            description_image: DescriptionImage = copy.deepcopy(
                self._description_image_base
            )
            self._description_images.append(description_image)

        # set each row value to each description image
        for description_index, row_ordered_dict in enumerate(
            get_list_of_ordered_dict_from_tsv(tsv_path)
        ):
            self._description_images[description_index].set_description_to_layers(
                row_ordered_dict
            )
    ...
    def make_images(self):
        ...
        if len(self._description_images) > 1:
            for description_image in self._description_images[1:]:
                ...
                # padding
                concatenated_image = get_h_concatenate_image(
                    concatenated_image, padding_image
                )
                concatenated_image = get_h_concatenate_image(
                    concatenated_image, description_image.img
                )
